...
from cache_sim.datasets.reader import Access, ContentTraceReader, check_trace

# 数据集根目录：项目根 / traces（本模块位于 cache_sim/datasets/loader.py，
# parents[2] 即项目根）
...
def find_dataset_path(name: str, split: str = "train") -> Optional[Path]:
    ...
    if name == "twitter29":
        ...
        candidates = [
            DATASETS_DIR / "twitter29" / "twitter29_test10.csv",
            DATASETS_DIR / "twitter29" / f"twitter29_{split}.csv",
        ]
    else:
        candidates = [
            DATASETS_DIR / name / f"{name}_{split}.csv",
            DATASETS_DIR / name / f"{name}_{split}_0.01.csv",  # 兜底：取小样本
        ]
    for c in candidates:
        if c.exists():
            return c
    # 兜底：在数据集目录下取任意匹配 split 的文件
    d = DATASETS_DIR / name
    if d.is_dir():
